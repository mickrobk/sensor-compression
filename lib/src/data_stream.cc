...
std::optional<tl::expected<CompressedDataFrame, std::string>> DataStream::Record(
    const DataHeader& header, DataFrameValue value) {
...
std::optional<tl::expected<CompressedDataFrame, std::string>> DataStream::Record(
    const DataHeader& header, uint value) {
